...
In practice, the $\dgr$ of reaction $i$ is calculated as the sum of the standard Gibbs' energy of reaction ($\sdgr$) and the scaled logarithm of products over reactants (\ref{eq:dgr})
\[
...
\]
% TODO: This is for the direct reaciton form, we should also have a form based on formation energies and highlight that they are equivalent
where $\mathbf{c} \in \mathbb{R}_{+}^m$ is the concentration of all metabolites $1,...,m$ in $S$.
This formulation also nicely highlights the fact that $\dgr$, $\$


% TODO: Find a nice formulation

% TODO: Learn a little more about activations vs concentrations. This should be in the equilibrator paper

In order to define this space, intracellular metabolite concentrations and standard Gibbs' energy ($\sdgr$) of reaction are required. %This needs to be exanded upon
% TODO This needs to flow into the next section

The estimation of the standard Gibb's energy of reactions ($\sdgr$) and standard Gibb's energy of formation ($\sdgf$) of compounds is essential for modelling accruarate intracellular fluxes.
As the $\sdgr$
