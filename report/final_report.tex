...
%Depending upon the amount of data available, this estimation process can range from a rough description limited only by fundamental stoichiometry and mass-balance laws, to a detailed flux space that respects the known kinetics and regulalatory interactions of metabolic enzymes.
%With increasing accuracy comes a concomitant increase in computational complexity in describing the space and finding individual solutions within it.
%In the case of steady-state flux modelling, Flux Balance Analysis (FBA) defines the space of valid fluxes as the null space of the stoichiometric matrix, that is, the space of fluxes that satisfy the steady-state condition.
%Obtaining a single solution from this space simply requires solving a linear system of equations, which can be solved at genome scale in less than a second.
%Fluxes can be further constrained to additionally satisfy thermodynamic laws.
%Obtaining a single solution from this, more complex, space, requires solving a Mixed Integer Linear Programming (MILP) problem, which can be solved at genome scale in the space of minutes to hours.
%The further addition of kintetic and regulatory information requires a system of ODEs {TODO: abbveriate?} to be solved {TODO: Is this faster?}.
%    {TODO: Maybe scrap this line of reasoning?}

Modelling of feasible intracellular fluxes has long been a key task in metabolic engineering.
Depending upon the amount of data available, this estimation process can range from a rough description limited only by fundamental stoichiometry and mass-balance laws, to a detailed flux space that respects the known kinetics and regulalatory interactions of metabolic enzymes.
A single most-parsimonious solution can then be obtained from this space, with parsiomny determined by maximising biomass production, nearness to measured data or other objective functions {TODO: Others?}.
...
An alternative approach is to characterise the whole space of vaiable fluxes, usually with some form of sampling.

Methods 1,2 {TODO: Research these} were developed to sample uniformly from the space of steady-state fluxes.
Saa {TODO: Citation} extended these methods to exclude loops from the flux space.
To remove the need for the explicit removal of loops, an alternative method is to include thermodynamic constraints on the space of valid fluxes.
In order to define this space, intracellular metabolite concentrations and standard Gibbs' energy ($\sdgr$) of reaction are required.
As these are both uncertain model parameters, some margin of error needs to be included to ensure that the true intracellular conditions can be represented.
...
% TODO: Brief interlude here about the probabilistic information we have about measurements and dgr estimates
...
However, both of these methods effectively throw away {TODO: Reformulate} useful probabistic information about these parameters, effectively treating all values within the confidence interval as equally likely.
...
% TODO: Talk here about why defining this space is helpful
