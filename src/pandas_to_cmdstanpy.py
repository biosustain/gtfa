...
from .util import codify, get_free_fluxes, rref
...
def extract_prior_1d(
    ...
    param_priors = (
        priors.groupby("parameter").get_group(parameter).set_index("target_id")
    )
    loc, scale = (
...
def extract_prior_2d(
    ...
    if parameter not in priors["parameter"].unique():
        ...
        loc, scale = (
            param_priors.set_index(["condition_id", "target_id"])[col]
            .unstack()
...
def get_exchange_rxns(S):
    ...
    EXCHANGE_NAMES = [
        re.escape(name)
        for name in ["transport", "exchange", "EX_", "SK_", "DM_", "EXCL_"]
    ]
...
def get_coords(S: pd.DataFrame, measurements: pd.DataFrame):
    ...
    assert is_exchange[: is_exchange.sum()].sum() == is_exchange.sum(), (
        "Exchange reactions should always be in the" " leftmost columns"
    )
...
def reorder_s(S):
    ...
    ordered = pd.concat(
        [
            pd.Series(S.columns[exchange_reactions]),
            pd.Series(S.columns[~exchange_reactions]),
        ]
    )
    S = S.loc[:, ordered]
...
def check_input(measurements, priors):
    ...
    if not measurements_by_type["enzyme"]["measurement"].between(0, 1).all():
        raise ValueError(
            "Enzyme concentration measurements should be between 0 and 1 molar"
            "Are they maybe recorded as log concentrations?"
        )
    if not measurements_by_type["mic"]["measurement"].between(0, 1).all():
        raise ValueError(
            "Metabolite concentration measurements should be between 0 and 1 molar. "
            "Are they maybe recorded as log concentrations?"
        )
    priors_by_type = dict(priors.groupby("parameter").__iter__())
    # Check that the lognormal priors are in the correct range
    if (
        "enzyme" in priors_by_type
        and not priors_by_type["enzyme"]["loc"].between(-20, 0).all()
    ) or (
        "concentration" in priors_by_type
        and not priors_by_type["concentration"]["loc"].between(-20, 0).all()
    ):
        raise ValueError(
            "Reasonable lognormal concentration priors should be between -20 and 0. "
...
def get_stan_input(
    ...
    free_exchange = get_name_ordered_overlap(
        coords, "reaction_ind", ["exchange", "free_x_names"]
    )
    free_met_conc = get_name_ordered_overlap(
        coords, "metabolite_ind", ["metabolite", "free_x_names"]
    )
    prior_b = extract_prior_2d(
        "b",
        priors,
        coords["internal_names"],
        coords["condition"],
        DEFAULT_B_MEAN,
        DEFAULT_B_SCALE,
    )
    prior_enzyme = extract_prior_2d(
        "internal_names",
        priors,
        coords["internal_names"],
        coords["condition"],
        DEFAULT_ENZ_CONC_MEAN,
        DEFAULT_ENZ_CONC_SCALE,
    )
    prior_met_conc_free = extract_prior_2d(
        "metabolite",
        priors,
        free_met_conc,
        coords["condition"],
        DEFAULT_MET_CONC_MEAN,
        DEFAULT_MET_CONC_SCALE,
    )
    prior_exchange_free = extract_prior_2d(
        "exchange",
        priors,
        free_exchange,
        coords["condition"],
        DEFAULT_EXCHANGE_MEAN,
        DEFAULT_EXCHANGE_SCALE,
    )
    ...
    return {
        ...
        "S": S.values.tolist(),
        # Indexing
        "ix_free_met_to_free": make_index_map(
            coords, "free_x_names", ["free_met_conc"]
        ),
        "ix_free_ex_to_free": make_index_map(
            coords, "free_x_names", ["free_exchange"]
        ),
        "ix_fixed_met_to_fixed": make_index_map(
            coords, "fixed_x_names", ["fixed_met_conc"]
        ),
        "ix_fixed_ex_to_fixed": make_index_map(
            coords, "fixed_x_names", ["fixed_exchange"]
        ),
        ...
        "ix_internal_to_rxn": make_index_map(
            coords, "reaction", ["internal_names"]
        ),
        "ix_ex_to_rxn": make_index_map(coords, "reaction", ["exchange"]),
        "ix_free_met_to_met": make_index_map(
            coords, "metabolite", ["free_met_conc"]
        ),
        "ix_fixed_met_to_met": make_index_map(
            coords, "metabolite", ["fixed_met_conc"]
        ),
        "ix_free_ex_to_ex": make_index_map(
            coords, "exchange", ["exchange", "free_x_names"]
        ),
        "ix_fixed_ex_to_ex": make_index_map(
            coords, "exchange", ["exchange", "fixed_x_names"]
        ),
        # measurements
        "N_condition": pd.concat(
            [measurements["condition_id"], priors["condition_id"]]
        ).nunique(),
        ...
        "sigma_flux": measurements_by_type["flux"][
            "error_scale"
        ].values.tolist(),
        "reaction_y_flux": measurements_by_type["flux"]["target_id"]
        .map(codify(coords["reaction"]))
        .values.tolist(),
        "condition_y_flux": measurements_by_type["flux"]["condition_id"]
        .map(codify(coords["condition"]))
        .values.tolist(),
        # Concentrations given on a log scale
        "y_enzyme": np.log(
            measurements_by_type["enzyme"]["measurement"]
        ).values.tolist(),
        "sigma_enzyme": measurements_by_type["enzyme"][
            "error_scale"
        ].values.tolist(),
        "internal_y_enzyme": measurements_by_type["enzyme"]["target_id"]
        .map(codify(coords["internal_names"]))
        .values.tolist(),
        "condition_y_enzyme": measurements_by_type["enzyme"]["condition_id"]
        .map(codify(coords["condition"]))
        .values.tolist(),
        # Concentrations given on a log scale
        "y_metabolite": np.log(
            measurements_by_type["mic"]["measurement"]
        ).values.tolist(),
        "sigma_metabolite": measurements_by_type["mic"][
            "error_scale"
        ].values.tolist(),
        "metabolite_y_metabolite": measurements_by_type["mic"]["target_id"]
        .map(codify(coords["metabolite"]))
        .values.tolist(),
        "condition_y_metabolite": measurements_by_type["mic"]["condition_id"]
        .map(codify(coords["condition"]))
        .values.tolist(),
        # priors
        ...
        "prior_exchange_free": [
            prior_exchange_free.location.values.tolist(),
            prior_exchange_free.scale.values.tolist(),
        ],
        "prior_enzyme": [
            prior_enzyme.location.values.tolist(),
            prior_enzyme.scale.values.tolist(),
        ],
        "prior_b": [
            prior_b.location.values.tolist(),
            prior_b.scale.values.tolist(),
        ],
        "prior_free_met_conc": [
            prior_met_conc_free.location.values.tolist(),
...
def get_name_ordered_overlap(
    coords: {}, order_by: str, name_list: [str]
) -> [int]:
    """
    ...
    return get_ordered_overlap(
        coords[order_by], [coords[name] for name in name_list]
    )
...
def get_ordered_overlap(order_by: {}, inds_lists: [list]) -> []:
    ...
    assert not any(
        [ind is None for ind in sorted_inds]
    ), "All indices should be in the sorting dict"
    return sorted_inds
...
def make_index_map(coords: dict, to_name: str, from_names: [str]):
    ...
    ordered_overlap = get_ordered_overlap(
        codified, [coords[name] for name in from_names]
    )
