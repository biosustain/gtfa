...
def test_model_writing(ecoli_model):
    ...
    assert all(
        ecoli_model.reactions[70].cal_stoichiometric_matrix() == S["FUMt2_2"]
    )
    ...
    components = np.concatenate(
        [tmet.compound_vector.T for tmet in ecoli_model.metabolites], axis=1
    )
    calc_cov = (
        components.T @ multitfa.util.thermo_constants.covariance @ components
    )
...
def test_model_writing_small(model_small):
    ...
    # Test the covariance matrix
    components = np.concatenate(
        [tmet.compound_vector.T for tmet in model_small.metabolites], axis=1
    )
    calc_cov = (
        components.T @ multitfa.util.thermo_constants.covariance @ components
    )
