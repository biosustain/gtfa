...
def run_standard_model():
    ...
    e = np.array(
        [
            0.003641157529874064,
            0.0023508286235591246,
            0.0004050401475330379,
            0.0006353773420867625,
            0.0006045197331044093,
        ]
    )
    write_measurements(test_dir, S, c, v, e, err=1)
    # Now run the sampling
    config = load_model_configuration(
        "/home/jason/Documents/Uni/thesis/gtfa/src/tests/test_small_likelihood.toml"
    )
...
def write_measurements(test_dir, S, c, v, e, err=1.0):
    ...
    enz_df = pd.DataFrame(
        zip(
            ["enzyme"] * num_internal,
            S.columns,
            ["condition_1"] * num_internal,
            e,
            [0.2 * err] * num_internal,
        ),
        columns=cols,
    )
