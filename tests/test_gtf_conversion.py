...
def test_small_model(model_small_rankdef_thermo):
    ...
    dgf0s, dgf0_cov = calc_model_dgf0s_with_prediction_error(model_small_rankdef_thermo)
    met_ids = [m.id for m in model_small_rankdef_thermo.metabolites]
    expected_dgf0s = pd.Series([-515.7, -508.1, -252.5, -272.1, -2263.6, -1388.7, -1055.5], index=met_ids)
    assert np.allclose(dgf0s, expected_dgf0s, atol=0.1)
    ...
    # assert np.allclose(np.sqrt(np.diag(dgf0_cov)), expected_sd, atol=0.1)
...
def test_small_model_different_conditions():
    ...
    expected_dgf0s = pd.Series([-510.3, -502.7, -237.6, -257.2, -2225.5, -1352.6, -1052.5,  # cytosol
